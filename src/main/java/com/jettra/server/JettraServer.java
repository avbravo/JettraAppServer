...
public class JettraServer {
    ...
    private com.jettra.server.core.AutocloneManager autocloneManager;
    ...
    public static String getContextPath() {
        String contextPath = com.jettra.server.config.JettraConfig.getProperty("server.contextpath");
    ...
    public void start() {
        ...
        String typeBackend = com.jettra.server.config.JettraConfig.getProperty("server.typebackend");
        if ("true".equalsIgnoreCase(typeBackend)) {
            ...
            com.jettra.server.autentification.repository.JettraSecurityDBInitializer.initializeIfEmpty();
        }

        // Add native admin console for security database
        this.addHandler("/securitydb/admin", new com.jettra.server.autentification.AdminConsoleHandler());

        // Inicializamos componentes abstractos/ejemplo si los hay
        System.out.println("Initializing ExampleRest and ConfigInjector...");
        com.jettra.server.test.ExampleRest example = new com.jettra.server.test.ExampleRest();
        com.jettra.server.config.ConfigInjector.inject(example);
        example.draw();
        ...
        try {
            ...
            if (customPort > 0) {
                ...
                String portValue = com.jettra.server.config.JettraConfig.getProperty("server.port");
            ...
            String compactHeader = com.jettra.server.config.JettraConfig.getProperty("server.compactheader");
            ...
            if (customPort <= 0) {
                String thresholdVal = com.jettra.server.config.JettraConfig.getProperty("server.autoclone.threshold");
                int threshold = (thresholdVal != null && !thresholdVal.isBlank()) ? Integer.parseInt(thresholdVal) : 100;
                autocloneManager = new com.jettra.server.core.AutocloneManager(this, threshold);
    ...
    public void stop() {
        ...
        com.jettra.server.core.JettraContext.clearSessions();
        System.out.println("[JettraServer] Todas las sesiones han sido cerradas.");

        String expiredValue = com.jettra.server.config.JettraConfig.getProperty("server.session.expired");
    ...
    private void startHotReloadWatcher() {
        String hotreload = com.jettra.server.config.JettraConfig.getProperty("server.hotreload");
